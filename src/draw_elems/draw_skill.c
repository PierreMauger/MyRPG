...
void draw_skill(game_t *game)
{
    ...
    while (temp != NULL) {
        ...
        if (check_collide_skill(game, temp)) {
            sfRenderWindow_drawSprite(game->window->window, temp->dsprite, NULL);
            sfRenderWindow_drawText(game->window->window, temp->text, NULL);
            if (temp->act_cd != 0)
                sfRenderWindow_drawText(game->window->window, temp->cd_text, NULL);
        }
        temp = temp->next;
        x += 100;
    }
