...
void draw_single_attak_target(game_t *game)
{
    ...
    sfText_setString(game->ind->damage, nbr_to_str(game->ind->ptr_skill->coef[game->ind->curr_attack]));
    sfText_setOrigin(game->ind->damage, (sfVector2f){sfText_getGlobalBounds(game->ind->damage).width / 2, 0});
    if (game->ind->ptr_skill->coef[game->ind->curr_attack] != 0 && game->ind->ptr_skill->arect.left == 320 - 80) //add image width to struct
