...
{
    sfVector2f mons_pos = sfSprite_getPosition(mons->texture->sprite);

    mons_pos.x -= mons->texture->rect.width / 2;
    mons_pos.y -= mons->texture->rect.height;
    if (game->mouse_pos.x > mons_pos.x && game->mouse_pos.x < mons_pos.x + mons->texture->rect.width && game->mouse_pos.y > mons_pos.y && game->mouse_pos.y < mons_pos.y + mons->texture->rect.height)
        return 1;
    return 0;
}
